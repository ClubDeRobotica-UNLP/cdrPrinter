...
void quickstop_stepper() {
   ...
   */
  inline void gcode_M420_Original() { if (code_seen('S') && code_has_value()) mbl.set_has_mesh(code_value_bool()); }
...
{
  uint8_t STAT_LED_RED = 4;
  uint8_t STAT_LED_BLUE = 5;
  uint8_t STAT_LED_GREEN = 6;
  
  if(millis() > stat_update)
  {
    stat_update += 50; // Update every 0.05s
    ExtTemp = thermalManager.degHotend(active_extruder);
    ExtTarg = thermalManager.degTargetHotend(active_extruder);
    BedTemp = thermalManager.current_temperature_bed;
    BedTarg = thermalManager.target_temperature_bed;
  ...
  //Fade Leds On when starting up
    ...
    if(Rval+Gval+Bval == 3)
    {
      ...
      AmbientTemp = thermalManager.degHotend(active_extruder);
    }

    if (ExtTemp < (AmbientTemp + 5))
    {
      white();

    } else if ((ExtTarg >= ExtTemp-TEMP_HYSTERESIS) && (ExtTarg <= ExtTemp+TEMP_HYSTERESIS)) {
